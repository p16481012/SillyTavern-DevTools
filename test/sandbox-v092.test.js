...
test('v0.15.7 sandbox exposes every browser review fixture deterministically', async () => {
    ...
    assert.match(harness, /extensionVersion:\s*'0\.15\.7'/);
    assert.match(harness, /privacy:\s*\{[\s\S]*?mode:\s*'full'/);
    assert.match(harness, /version:\s*'0\.15\.7'/);
