...
test('v0.14.0 sandbox exposes every browser review fixture deterministically', async () => {
    ...
    assert.match(harness, /extensionVersion:\s*'0\.14\.0'/);
    assert.match(harness, /privacy:\s*\{[\s\S]*?mode:\s*'full'/);
    assert.match(harness, /version:\s*'0\.14\.0'/);
    ...
    assert.match(html, /ST DevTools v0\.14\.0 UI Sandbox/);
