...
test('v0.16.10 sandbox exposes every browser review fixture deterministically', async () => {
    ...
    assert.match(harness, /extensionVersion:\s*'0\.16\.10'/);
    assert.match(harness, /privacy:\s*\{[\s\S]*?mode:\s*'full'/);
    assert.match(harness, /version:\s*'0\.16\.10'/);
    ...
    assert.match(harness, /SEMANTIC_RATE_LIMITED/);
    assert.match(harness, /provider-rate-limited/);
    assert.match(harness, /semanticInspector:\s*sandboxSemanticInspector/);
    assert.match(harness, /new SemanticProviderEvaluationHarness\(/);
    assert.match(harness, /semanticEvaluationHarness:\s*sandboxSemanticEvaluationHarness/);
    ...
    assert.match(html, /ST DevTools v0\.16\.10 UI Sandbox/);
