...
이 문서는 v0.15.5 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.5는 기존 읽기 전용 탐색·복사·AI 동의 경계와 분리 더미 session의 6개 주제·38단계를 유지하면서 별도 설명 sheet를 실제 화면 위 코치마크로 교체합니다. 암전 상태에서는 target ring·점선 화살표·제목과 한 문장만, 밝은 체험 상태에서는 실제 target pulse와 한 줄 지시만 보여 줍니다. 읽기 단계는 한 번의 안내로 끝나고 하단에는 원형 종료·다음 버튼과 진행 숫자만 남습니다.
