...
이 문서는 v0.11.1 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.11.1은 캡처한 요청을 읽기 전용으로 탐색·비교하고, 명시적인 지시를 원자 구조로 보존한 뒤 실제 충돌 쌍과 클러스터를 검사합니다. 전체·프리셋·캐릭터·채팅 정책 프로필, 재사용 그룹 동작, 이름 규칙과 수동 지정으로 비교군을 구조화하며, 사용자가 검사 결과를 판정·숨김·범위별 무시하고 정책 변경 전후를 검토할 수 있습니다. 4개 작업 중심 탐색, 빠른 시작·도움말과 캡처 상태 표시로 처음 사용할 때의 진입점을 줄였습니다.
...
선택적 AI Semantic Inspector는 사용자가 직접 고른 정적 finding·cluster의 로컬 근거만 매 호출 동의 후 현재 provider에 보냅니다. 엄격한 JSON·근거 offset 검증을 통과한 제안도 정적 결과와 분리하며 자동 수정·판정·정책 변경에는 사용하지 않습니다. 따라서 v0.11.1도 자연어 의미 전체나 프롬프트 품질을 보증하지 않습니다.
...
v0.12.0은 새 대형 편집 기능보다 v0.11.1의 실제 품질을 측정하고 설명 가능성을 높이는 방향으로 진행합니다.

- 익명화 평가 corpus와 AI 제안 유용성·오탐·근거 정확성 회귀 기준
- provider 호환성, 오류 분류, 성능·접근성·모바일 품질 보강
- 모바일·키보드·screen reader 접근성과 4개 작업 탐색·도움말의 실제 사용성 검증

세부 범위는 v0.11.1 대규모 사용자 검토 결과로 확정합니다. 코치마크·워크스루는 이번 정보 구조가 실제 사용자에게도 이해되는지 확인한 뒤 별도 범위로 구체화합니다.
