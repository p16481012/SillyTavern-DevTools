...
이 문서는 v0.15.7 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.7은 기존 읽기 전용 탐색·복사·AI 동의 경계와 분리 더미 session의 6개 주제·38단계를 유지하면서 반복적인 체험 진입 클릭을 제거합니다. 읽기 전용 설명은 암전 briefing과 `다음` 한 번을 사용하고, 조작 단계는 밝은 practice에서 기능 의미·사용 맥락·할 일과 실제 control을 곧바로 함께 보여 준 뒤 debrief의 녹색 result spotlight·`잘했어요!`·관찰 결과로 이어집니다.
