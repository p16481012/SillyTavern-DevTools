...
이 문서는 v0.12.4 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.
...
- v0.12.4 실사용에서 발견되는 성능·접근성·모바일 UI/UX 불편 보완

세부 범위는 v0.12.4 대규모 사용자 검토 결과로 확정합니다. 코치마크·워크스루는 평가 기준과 접근성 계약이 정리된 뒤 별도 버전으로 계획합니다.
