...
이 문서는 v0.15.2 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.2는 기존 읽기 전용 탐색·복사·AI 동의 경계와 6개 주제·38단계 hands-on 실습을 유지하면서, 강조 표시가 스크롤을 따라가고 안내 패널이 대상 주변의 가용 공간으로 이동하도록 보완합니다. 대상이 화면 밖이면 복귀 컨트롤로 전환하고, 핵심 행동과 보충 설명을 분리해 첫 화면의 읽기 부담을 줄였습니다.
