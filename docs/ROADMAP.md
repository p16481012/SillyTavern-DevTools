...
이 문서는 v0.16.0 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.16.0은 기존 읽기 전용 탐색·복사·AI 동의 경계와 분리 더미 session의 6개 주제·38단계를 유지하면서 `기능 도움말 골격 → 비교 정책 연습실 → AI 의미 검사 시뮬레이션 → 나머지 기능 문서` 순서를 완료합니다. 같은 source의 수정과 대안 그룹 옵션 교체도 변경 비교에서 별도 의미로 표시합니다.
