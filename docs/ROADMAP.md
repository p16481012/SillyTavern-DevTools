...
이 문서는 v0.13.1 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.13.1은 캡처한 요청을 읽기 전용으로 탐색·비교하고, 각 프롬프트 원문과 생성 설정·payload, AI 제안을 복사해 SillyTavern의 원래 편집 화면에 붙여넣을 수 있게 합니다. 직접 저장·자동 적용 기능은 추가하지 않습니다. 캐릭터 설명·성격과 사용자 페르소나가 같은 프로필 구성을 쓰는 것만으로 중복·충돌을 만들지 않으며 시나리오·첫 메시지·시스템 프롬프트의 실제 지시는 계속 비교합니다.
...
## v0.14.0 후보 — 평가 확대·선택형 온보딩

- 합성 corpus의 조건·예외·말투·역할·안전 의미 사례 확대와 실제 provider 수동 평가 절차 정리
- 다섯 기능 정보 구조를 처음 쓰는 사용자를 위한 건너뛸 수 있는 코치마크·워크스루 설계
- 키보드·screen reader·모바일에서 같은 순서와 종료·재시작 경계를 먼저 검증

v0.14.0 세부 범위는 v0.13.1 실사용 검토 결과로 확정합니다. 코치마크는 프롬프트 원문이나 AI 전송을 자동 실행하지 않습니다.
