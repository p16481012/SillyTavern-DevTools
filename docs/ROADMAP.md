...
이 문서는 v0.17.0 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.17.0은 Rule Inspector의 조건·예외 적용 범위를 1차 구조화합니다. 같은 단순 조건에서 겹치는 반대 지시는 충돌로, 서로 배타적인 조건과 일반 규칙의 예외를 담당하는 특수 규칙은 별도 양립 관계로 판정합니다. 양립 관계는 충돌 finding·cluster에서 제외하되 `조건상 양립` 통계와 근거로 확인할 수 있고, 적용 범위 메타데이터는 선택적 AI 입력까지 유지됩니다.

## 다음 개발 방향

### v0.17.1 — 말투·정체성·안전·메모리 의미 범위 · 예정

- 의미 축을 한꺼번에 자연어 전체 이해로 확장하지 않고 말투·정체성·안전·메모리별 taxonomy와 release gate를 분리
- 캐릭터와 페르소나처럼 구조는 비슷하지만 참가자와 대상이 다른 프롬프트를 충돌로 보지 않도록 scope를 먼저 판정
- 실제 우선순위 승자 추정은 근거가 충분한 별도 단계 전까지 제외
