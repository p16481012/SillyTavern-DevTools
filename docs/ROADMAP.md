...
이 문서는 v0.16.2 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.16.2는 세 갈래 도움말 구조를 유지하면서 기능 설명서 19개 전체를 시각 도식으로 보강하고 고급 기능 가이드를 다섯 주제로 확장합니다. 완료 카드 대비와 검색 모바일 배치를 보정하고, 비교 정책·AI 의미 검사·판정과 예외·Atom/Relation·대안 교체를 실제 화면형 더미 코치마크로 안내합니다.
