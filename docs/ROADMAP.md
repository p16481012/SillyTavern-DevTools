...
이 문서는 v0.15.0 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.0은 기존 읽기 전용 탐색·복사·AI 동의 경계를 유지하면서 처음 쓰는 사용자가 다섯 기능을 실제와 닮은 연습 데이터로 익히는 선택형 워크스루를 추가합니다. 실제 provider 호출과 저장 데이터 변경 없이 캡처부터 검색까지 결과를 읽는 순서를 안내합니다.

## v0.15.0 — 선택형 첫 사용 워크스루 · 완료

- 첫 실행 1회 제안, 명시적 건너뛰기, 완료, 헤더 나침반 버튼 재시작
- 캡처 → 프롬프트 → 규칙 검사 → 기록 → 변경 비교 → 검색의 6단계 정보 구조
- 실제 화면 대상 spotlight와 대상이 없을 때 중앙 fallback
- provider/model/토큰·포함 상태·충돌 근거·토큰 급증·diff·검색 일치의 구체적인 고정 연습 데이터
- 모든 단계의 `연습 데이터 · 저장·전송 안 함` 표기와 실제 tab·selection·snapshot·설정·저장소 무변경
- AI 의미 검사·공식 Provider 평가와 동시 실행 금지, provider·분석·clipboard·export 호출 0 경계
- `skipped`·`completed`만 기록하는 버전 지정 최소 상태; 진행 단계·시각·내용 미저장
- Escape·이전·다음·완료, focus trap·복원·screen reader 상태, 모바일 bottom sheet
- 샌드박스 전용 진입점과 호출 0·레이아웃·재시작 회귀 검증
...
- 실제 초보자 검토에서 막히는 단계의 문구·예시 밀도·강조 위치 보정
- 제품 기능 추가 시 새 단계를 무조건 늘리지 않고 기존 여섯 단계 안에서 가장 가까운 개념에 편입
- 실제 데이터 미사용·자동 AI 호출 금지·최소 상태 저장 경계 유지

코치마크는 프롬프트 원문이나 AI 전송을 자동 실행하지 않습니다.
