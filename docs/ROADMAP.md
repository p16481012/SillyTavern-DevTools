...
이 문서는 v0.9.1 코드와 자동 테스트를 기준으로 남은 기능을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.9.1은 캡처한 요청을 읽기 전용으로 탐색·비교하고, 명시적인 지시를 원자 구조로 보존한 뒤 실제 충돌 쌍과 클러스터를 검사합니다. 지시·참고·대화·도구·멀티모달 소스 capability를 분리하고 확정·후보·근거 부족 판정과 source·range·method·confidence 근거를 표시합니다. 전체·프리셋·캐릭터·채팅 정책 프로필, 재사용 그룹 동작, 이름 규칙과 수동 지정으로 비교군을 구조화하며, 사용자가 검사 결과를 판정·숨김·범위별 무시하고 정책 변경 전후를 검토할 수 있습니다. 스냅샷은 개별 레코드로 저장되고 보관 수와 패널이 읽는 최신 범위를 사용자가 따로 정합니다.
