...
이 문서는 v0.17.3 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.17.3은 여러 소스의 동일 문장을 source별 근거 카드와 정확한 local/final range로 분리합니다. 각 근거에서 원본 소스와 최종 위치를 따로 열 수 있고, source 수와 물리적으로 서로 다른 최종 위치 수를 구분합니다. 같은 source 안의 반복은 하나로 취합하면서 전체 위치와 횟수를 요약으로 보존합니다. 이동용 범위는 source 20곳·source별 local/final 각 100곳으로 제한하고 생략 수를 표시하며, v0.17.2 이전에 저장한 유효·오탐 판정의 review key는 유지됩니다.

## 다음 개발 방향

### v0.17.4 — 복합 조건 적용 범위 확장 후보 · 검토

- v0.17.0의 단순 equality·정확 절 비교를 넘는 `and`·`or` 조건의 포함·배타 판정은 별도 corpus와 오탐 budget을 통과할 때만 확장
- 자연어 전체 논리 추론이나 실제 우선순위 승자 추정과 분리해 fail-closed 후보 상태를 유지

## v0.17.3 — 여러 소스의 중복 문장 근거 분리 · 완료

- 동일 문장의 source별 원문과 정확한 source 내부 범위·최종 프롬프트 범위를 별도 evidence record로 보존
- 결과 카드에서 source별 근거를 나누고 각 근거의 원본 소스와 최종 위치를 독립적으로 여는 action 제공
- 원본 source 수와 중복을 제거한 물리적 최종 위치 수를 따로 표시하고 여러 source가 같은 최종 위치를 공유하는 상황을 설명
- 한 source 안의 동일 문장 반복은 한 finding·한 카드로 취합하고 전체 횟수·위치 수는 summary에 유지하되, 강조 범위는 local/final 각각 최대 100곳으로 제한하고 생략 수를 표시
- 새 위치 metadata를 review key에서 제외해 기존 유효·오탐 판정과 suppression 호환성을 유지
- 같은 앞부분을 가진 서로 다른 문장 ID 충돌, 여러 가상 source mount, 다중 원문 강조를 회귀 대상으로 고정

## v0.17.2 — 검사 결과에서 행동까지의 흐름 · 완료

- 발견 이유 → 최대 두 atom 근거 비교 → 개별/함께 원문 위치 열기 → 검사 finding 복귀를 하나의 카드 흐름으로 연결
- 유효·오탐과 이번만 숨김·범위별 항상 무시를 기존 검토 저장 경계에 연결하고 keyboard focus를 원래 카드로 복원
- 정확히 한 finding만 선택해 AI 준비 화면으로 이동하고 AI 결과에서 로컬 검사로 복귀하는 명시적 경로 추가
- 검증된 제안의 제목·요약·판단 이유만 클립보드로 복사하고 원본 프롬프트·정적 finding·정책·판정을 자동 수정하지 않음
- AI 준비와 provider 실행을 분리해 기존 미리보기·매회 동의 전에는 네트워크를 호출하지 않음
- 320px·390px 한 열 fallback, 넓은 화면의 두 근거 열, 44px 조작 영역, focus 복원, 동작 줄이기와 개인정보 모드 경계를 회귀 대상으로 고정

## v0.17.1 — 적용 주체·말투·정체성·안전·메모리 · 완료

- atom에 적용 주체를 추가하고 source origin과 행동 주체를 분리해 캐릭터·페르소나 구조 유사 오탐을 차단
- 말투 3축, 명시적 배타 정체성, 비밀 공개·유해 상세 안전 지시, 이전 대화 사용·민감 정보 보관을 bounded taxonomy로 구조화
- 주체 미확인, 다른 주체, 다른 속성·기억 대상은 비교하지 않고 자유 서술 역할은 기존 `근거 부족` 경계를 유지
- 신규 축을 Rule Inspector finding·설정·비교 정책·검토 무시·한글 UI·선택적 AI closure에 연결
- 공식 Provider 평가를 18건·6축·최대 54회로 확대하고 양성·음성 대조군과 구조 제품 경로를 고정
- 실제 우선순위 승자, 자유 자연어 함의, 복합 조건 전체 논리, 자동 수정은 범위에서 제외

## v0.17.0 — Rule Inspector 조건·예외 관계 1차 · 완료

- 조건·예외 원문과 bounded predicate를 원자에 함께 보존하고 관계별 적용 범위를 `무조건`, `같은 조건`, `부분 범위`, `배타 조건`, `예외 특수화`, `판단 보류`로 분류
- 같은 조건의 반대 지시는 확정 충돌, 서로 배타적인 단순 조건과 예외 특수화는 확정 양립으로 판정
- 복합·불명 조건은 충돌 후보로 남겨 정적 분석이 확인하지 못한 양립을 추측하지 않음
- 양립 관계를 충돌 relation·cluster·finding에서 분리하고 100개 상한, 생략 상태, UI 통계·상세 근거를 추가
- finding과 선택적 AI 입력에 `applicabilityKind`·`relationDisposition` 및 조건·예외를 보존하고 잘못된 enum은 실패로 닫음
- 같은 조건 충돌·배타 조건 양립·예외 특수화 양립·복합 조건 후보·finding 제외를 golden과 제품 경로 테스트로 고정

## v0.16.11 — 탭별 빈 상태·스크롤 폭 안정화 · 완료

- 다섯 탭의 제목·설명·아이콘·다음 행동을 기능별로 분리하고 빠른 시작 카드는 전송 프롬프트에만 유지
- 변경 비교는 현재 불러온 요청 수를 `0/2`·`1/2`로 표시하고 저장 총수가 충분한 경우 불러오기 설정으로 연결
- 문제 해결과 새로고침을 하나의 공통 disclosure로 묶고 일반 빈 화면의 주 행동은 채팅 복귀로 단순화
- `.st-devtools-content.is-empty-state`에서 scrollbar 표시만 숨기고 `overflow-y: auto`를 유지해 폭 흔들림 없이 실제 스크롤 보존
- 390×640 밝음·어두움 샌드박스에서 토글 전후 `clientWidth`·카드 위치 일치, `scrollWidth === clientWidth`, 휠 스크롤을 검증

## v0.16.10 — 튜토리얼 암막 안정화·도움말 재구성 · 완료

- 전체 화면 shadow에 적용되던 opacity·shadow·filter 전환 제거
- 펼침 중 실제 details geometry만 추적하고 중복 scroll·spotlight easing 제거
- 고급 기능 가이드 목차 여백과 모바일 터치 영역 확장
- 기능 설명서를 카테고리 목차와 번호형 `설명 → 실제 UI` 본문으로 재구성
...
- 실제 `index.js`가 `ui.js?v=<확장 버전>`을 import하고 manifest·package·runtime·sandbox와 같은 버전인지 회귀 검증
- 세로 smooth scroll 직후 별도 `scrollLeft` 대입으로 네이티브 이동을 취소할 수 있는 경합 제거
- 자동 스크롤을 시작 당시 단계·대상과 연결하고 완료 시점에 안전 영역을 재검사해 오래된 `scrollend`가 새 단계에 간섭하지 않도록 제한
- 영상과 동일한 프롬프트 13단계의 포함 필터 완료 → 프리셋 묶음 진입 → disclosure 펼침 경로를 모바일 샌드박스에서 검증
- 다음 후보: 실제 호스트 테마가 content 바깥 요소를 scroll owner로 바꾸는 사례를 수집해 scroll owner 탐색 범위 확장
...
- 빈 기록 화면의 disclosure·버튼 영역 간격과 설명 본문·CTA 내부 여백 보강
- 기본 사용법 전체 시작 CTA의 full-width 46px 높이와 기능 설명서 카테고리·카드·본문 간격 표준화
- 기능 설명서 19개 도식을 실제 제품의 header·card·status badge·group surface와 닮은 정적 더미 화면으로 전환
- 390px 패널에서 기능 설명서 preview의 한 열 fallback과 수평 overflow 없음 확인
- 튜토리얼 전용 이름 규칙으로 `예시 수 | 1개 → 2개`를 같은 대안 그룹의 1:1 `교체`로 판정
- 기본 비교를 8단계, 전체 안내를 39단계로 확장하면서 실제 저장 정책·store·provider 비접근 유지
- 다음 후보: 사용자 검토에서 제품형 화면 예시의 정보 밀도와 상태 문구를 보정하고, 자주 찾는 설명서 항목의 순서·검색 동의어를 실제 사용 빈도에 맞춰 조정

## v0.16.2 — 설명서 도식·고급 가이드 확장·튜토리얼 안정화 · 완료

- 완료 debrief의 불투명 고대비 surface와 target 분리로 아래 글자 비침·copy 겹침 보정
- 검색 튜토리얼의 넓은 snapshot select와 coachmark를 패널 가시 폭 안에 제한해 모바일 좌측 쏠림·가로 overflow 방지
- 도움말 홈 카드의 중복 파란색 `… 보기` copy 제거와 카드 전체 진입 control 유지
- 19개 기능 설명서에 외부 이미지 없이 렌더링하는 흐름·대조·분기·병렬 레인 도식 제공
- 비교 정책을 적용 범위·그룹 동작·이름 규칙·수동 지정·우선순위·전후 결과의 9단계 가이드로 확대
- AI 의미 검사를 후보 선택·연결·응답 상한·prompt/prefill·전송 범위·동의·검증·안전 폐기의 10단계 가이드로 확대
- 검사 결과 판정과 예외 관리 7단계, Atom·Relation 근거 읽기 7단계, 대안 그룹 교체 비교 6단계 추가
- 다섯 고급 guide의 실제 정책·검토 결정·snapshot store·provider·비용·전역 온보딩 상태 비접근 보장
- 다음 후보: 사용자 실사용에서 자주 찾는 설명서 순서·검색어 보정, 문서 도식의 실제 화면 위치 이동, 더미 사례의 현업 프롬프트 유형 확대
...
- 제품 기능 추가 시 새 단계를 무조건 늘리지 않고 기존 6개 주제·39단계 중 가장 가까운 개념에 우선 편입
