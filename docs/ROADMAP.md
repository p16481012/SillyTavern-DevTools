...
이 문서는 v0.16.3 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.16.3은 세 갈래 도움말 구조를 유지하면서 빈 기록 화면·기본 사용법 CTA·기능 설명서 목록의 세로 리듬을 보정합니다. 기능 설명서 19개는 번호형 도식 대신 제품 카드와 닮은 읽기 전용 더미 화면을 사용하며, 기본 변경 비교는 전용 대안 정책으로 `교체`까지 포함한 8단계·전체 39단계를 결정적으로 재현합니다.
