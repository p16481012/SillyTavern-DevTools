...
이 문서는 v0.11.2 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.11.2는 캡처한 요청을 읽기 전용으로 탐색·비교하고, 명시적인 지시를 원자 구조로 보존한 뒤 실제 충돌 쌍과 클러스터를 검사합니다. 전체·프리셋·캐릭터·채팅 정책 프로필, 재사용 그룹 동작, 이름 규칙과 수동 지정으로 비교군을 구조화하며, 사용자가 검사 결과를 판정·숨김·범위별 무시하고 정책 변경 전후를 검토할 수 있습니다. 4개 작업 중심 탐색, 빈 상태 빠른 시작·필드별 도움말과 캡처 상태 표시로 처음 사용할 때의 진입점을 줄였습니다. 헤더는 새로고침·설정·닫기 세 동작만 유지합니다.
...
선택적 AI Semantic Inspector는 사용자가 직접 고른 정적 finding·cluster의 로컬 근거만 매 호출 동의 후 선택한 SillyTavern Connection Manager 프로필 또는 현재 provider에 보냅니다. 설정에는 불투명 profile ID만 남기며 API 키·URL·연결 비밀값을 저장하지 않습니다. 엄격한 JSON·근거 offset 검증을 통과한 제안도 정적 결과와 분리하며 자동 수정·판정·정책 변경에는 사용하지 않습니다. 따라서 v0.11.2도 자연어 의미 전체나 프롬프트 품질을 보증하지 않습니다.
...
v0.12.0은 새 대형 편집 기능보다 v0.11.2의 실제 품질을 측정하고 설명 가능성을 높이는 방향으로 진행합니다.
...
세부 범위는 v0.11.2 대규모 사용자 검토 결과로 확정합니다. 현재 빠른 시작과 tooltip은 완료된 온보딩으로 간주하지 않습니다. 코치마크·워크스루는 이번 정보 구조가 실제 사용자에게도 이해되는지 확인한 뒤 별도 범위로 구체화합니다.
