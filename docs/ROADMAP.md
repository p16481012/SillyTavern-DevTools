...
이 문서는 v0.15.1 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.1은 기존 읽기 전용 탐색·복사·AI 동의 경계를 유지하면서 처음 쓰는 사용자가 실제 제품 UI를 직접 조작하는 선택형 hands-on 실습을 제공합니다. 8~12분 동안 캡처와 다섯 제품 화면을 아우르는 6개 주제·38단계를 진행하되, 실제 데이터 대신 별도 더미 세션을 활성 뷰에 주입합니다.
