...
이 문서는 v0.13.0 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.
...
v0.14.0 세부 범위는 v0.13.0 실사용 검토 결과로 확정합니다. 코치마크는 프롬프트 원문이나 AI 전송을 자동 실행하지 않습니다.
