...
이 문서는 v0.16.11 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.16.11은 저장 데이터가 없을 때 다섯 기능이 각자 필요한 조건과 다음 행동을 안내합니다. 변경 비교는 불러온 수와 저장 총수를 함께 사용해 `0/2`·`1/2`와 읽기 수 부족을 구분하고, 빈 화면의 문제 해결 disclosure가 길어져도 스크롤 rail만 숨겨 콘텐츠 폭과 실제 스크롤을 모두 유지합니다.

## 다음 개발 방향

### v0.17.0 — Rule Inspector 조건·예외 관계 1차 · 예정

- 같은 조건에서 충돌하는 지시, 서로 배타적인 조건이라 양립하는 지시, 일반 규칙을 좁히는 예외를 서로 다른 relation으로 판정
- 조건의 동치·포함·배타를 확정할 수 없으면 충돌 확정 대신 후보 또는 근거 부족으로 유지
- 한국어·영어 양성/음성 corpus와 제품 경로 fixture, 오탐 budget을 먼저 고정
