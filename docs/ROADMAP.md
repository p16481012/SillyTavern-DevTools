...
이 문서는 v0.15.8 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.8은 기존 읽기 전용 탐색·복사·AI 동의 경계와 분리 더미 session의 6개 주제·38단계를 유지하면서 이전 단계 이동과 변경 비교 강조를 보완합니다. 하단 기능 탭의 실제 높이를 기준으로 이전·다음 control과 본문 안전 공간을 계산하고, 비동기 comparison renderer가 target을 만든 뒤에도 practice spotlight를 다시 연결합니다.
