...
이 문서는 v0.17.2 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.17.2는 v0.17.1의 적용 주체·의미 축 위에서 Rule Inspector 결과를 읽고 행동하는 경로를 완성합니다. finding의 발견 이유와 최대 두 근거를 비교하고 실제 원문 위치를 확인한 뒤 검사 카드로 돌아와 유효·오탐·무시를 선택하거나 정확히 한 finding만 AI 의미 검사에 준비할 수 있습니다. AI는 자동 호출되지 않고, 검증된 제안 설명은 클립보드로만 복사되며 원본 프롬프트·정책·판정은 자동 변경되지 않습니다.

## 다음 개발 방향

### v0.17.3 — 복합 조건 적용 범위 확장 후보 · 검토
