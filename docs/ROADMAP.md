...
이 문서는 v0.16.10 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.16.10은 기존 선배치 순서를 유지하면서 disclosure 전환 중 전체 암막의 밝기와 viewport를 고정합니다. 도움말은 고급 가이드 카드 목차와 접을 수 있는 기능 설명서 카테고리, 설명을 먼저 읽고 실제 UI를 확인하는 번호형 상세 본문으로 정리합니다.
