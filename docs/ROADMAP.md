...
이 문서는 v0.11.4 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.11.4는 캡처한 요청을 읽기 전용으로 탐색·비교하고, 실제 SillyTavern 요청의 `undefined` 선택 필드를 정규화한 최소 스냅샷을 먼저 저장·재읽기 검증한 뒤 상세 소스 귀속을 같은 ID로 보강합니다. 명시적인 지시를 원자 구조로 보존한 뒤 실제 충돌 쌍과 클러스터를 검사하고, 전체·프리셋·캐릭터·채팅 정책 프로필, 재사용 그룹 동작, 이름 규칙과 수동 지정으로 비교군을 구조화합니다. 4개 작업 중심 탐색, 빈 상태 빠른 시작·필드별 도움말과 단계별 캡처 상태 표시를 유지합니다.
...
선택적 AI Semantic Inspector는 사용자가 직접 고른 정적 finding·cluster의 로컬 근거만 매 호출 동의 후 선택한 SillyTavern Connection Manager 프로필 또는 현재 provider에 보냅니다. 설정에는 불투명 profile ID만 남기며 API 키·URL·연결 비밀값을 저장하지 않습니다. 엄격한 JSON·근거 offset 검증을 통과한 제안도 정적 결과와 분리하며 자동 수정·판정·정책 변경에는 사용하지 않습니다. 따라서 v0.11.4도 자연어 의미 전체나 프롬프트 품질을 보증하지 않습니다.
...
v0.12.0은 새 대형 편집 기능보다 v0.11.4의 실제 품질을 측정하고 설명 가능성을 높이는 방향으로 진행합니다.

- 익명화 평가 corpus와 AI 제안 유용성·오탐·근거 정확성 회귀 기준
- provider 호환성, 오류 분류, 성능·접근성·모바일 품질 보강
- 모바일·키보드·screen reader 접근성과 4개 작업 탐색·빠른 시작·필드별 도움말의 실제 사용성 검증

세부 범위는 v0.11.4 대규모 사용자 검토 결과로 확정합니다. 현재 빠른 시작과 tooltip은 완료된 온보딩으로 간주하지 않습니다. 코치마크·워크스루는 이번 정보 구조가 실제 사용자에게도 이해되는지 확인한 뒤 별도 범위로 구체화합니다.
