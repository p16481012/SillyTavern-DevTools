...
이 문서는 v0.17.1 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.17.1은 v0.17.0의 조건·예외 관계 위에 적용 주체와 말투·배타적 정체성·안전·메모리 축을 추가합니다. 다른 참가자나 미확인 주체는 서로 비교하지 않고, 같은 주체·같은 의미 속성의 명시적인 반대 지시만 relation으로 만듭니다. 신규 축은 로컬 finding·설정·검토 fingerprint·선택적 AI 입력과 공식 18건 Provider 평가까지 동일한 구조로 연결됩니다.

## 다음 개발 방향

### v0.17.2 — 검사 결과에서 행동까지의 흐름 · 예정

- 발견 이유 → 두 근거 위치 → 비교 → 유효·오탐·무시 판정 → 개선 제안 복사를 한 결과 흐름으로 연결
- 원본 자동 수정이나 덮어쓰기는 추가하지 않고 읽기 전용·클립보드 복사 경계를 유지
