...
이 문서는 v0.15.6 코드 기준으로 완료 범위와 다음 방향을 나눈 예상 계획입니다. 버전 번호는 기능 의존 관계를 나타내며 일정 약속은 아닙니다.

## 현재 기준선

v0.15.6은 기존 읽기 전용 탐색·복사·AI 동의 경계와 분리 더미 session의 6개 주제·38단계를 유지하면서 레퍼런스형 코치마크의 설명·상태 동기화·결과 feedback을 보완합니다. 암전 briefing은 기능 의미와 사용 맥락을 두 문장으로, 밝은 practice는 실제 control과 짧은 지시를, debrief는 녹색 result spotlight·`잘했어요!`·관찰 결과를 표시합니다.
