...
9. 토큰 수·소스 연결 상태·멀티모달 토큰 추정치를 비동기로 계산한 뒤 현재 채팅 타임라인에 저장합니다.
10. 같은 채팅의 저장·읽기·삭제·비우기는 채팅 키 잠금으로 직렬화하고, 채팅 인덱스 갱신도 해당 작업 안에서 완료합니다.
11. 저장에 실패하면 계산을 마친 동일 스냅샷과 오류를 UI에 전달하여 같은 ID로 재시도합니다.
...
스토리지를 읽을 때 스키마 v1~v3 스냅샷을 v4로 한 번 변환하고 같은 타임라인에 다시 저장합니다. 기존 payload·최종 텍스트·ID는 변경하지 않으며, 과거에 캡처하지 않은 요청 본문과 멀티모달 크기는 비어 있는 상태로 유지합니다. v0.8.4의 `provider`는 v4에 추가 가능한 필드이며, 필드가 없는 기존 스냅샷은 저장된 요청 설정의 `chat_completion_source`를 읽고 그것도 없을 때만 `api`로 대체합니다.
...
3. `{group}`·`{option}` 템플릿 또는 정규식 capture를 정규화된 그룹 키로 변환합니다.
