# v0.11.4 사용자 실사용 체크리스트

v0.11.4는 실제 SillyTavern 요청의 선택적 `undefined` 값을 저장 가능한 형태로 정규화하고, 상세 소스 분석보다 최소 스냅샷의 영구 저장과 재읽기 검증을 먼저 수행합니다. 아래 항목은 실제 SillyTavern의 현재 provider/model, 연결 프로필, 채팅 캡처, IndexedDB와 브라우저 UI에서 확인해야 하는 검토 절차입니다. AI 검사는 선택한 full snapshot의 프롬프트 원문을 선택한 프로필 또는 현재 provider에 보낼 수 있고 취소가 이미 시작된 계산·과금을 되돌린다고 보장하지 않으므로, 비밀이나 실제 개인정보가 없는 테스트 프롬프트와 작은 응답 상한을 사용하세요.

## 시작 전

- [ ] ST DevTools 제목에 `v0.11.4`가 표시된다.
...
v0.12.0은 새 대형 편집 기능보다 익명화 평가 corpus, AI 제안·근거 품질 회귀, provider 호환성과 모바일·접근성 품질을 보강하는 방향입니다. 현재 빠른 시작과 필드별 tooltip은 완료된 온보딩 튜토리얼이 아닙니다. 코치마크·워크스루는 이 체크리스트의 v0.11.4 실사용 피드백으로 작업 구분과 안내 진입점이 적절한지 확인한 뒤 구체화합니다.
