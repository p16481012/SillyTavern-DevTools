# v0.12.1 사용자 실사용 체크리스트

v0.12.1은 4개 그룹과 조건부 하위 탭을 폐기하고 여섯 실제 기능을 고정 하단 내비게이션에서 직접 선택하도록 교정합니다. 아래에는 v0.12.1 단층 탐색·모바일 앱 셸 검토와 v0.11.4 이하 캡처·AI·저장 회귀 검토를 함께 적었습니다. AI 검사는 선택한 full snapshot의 프롬프트 원문을 선택한 프로필 또는 현재 provider에 보낼 수 있고 취소가 이미 시작된 계산·과금을 되돌린다고 보장하지 않으므로, 비밀이나 실제 개인정보가 없는 테스트 프롬프트와 작은 응답 상한을 사용하세요.

## 시작 전

- [ ] ST DevTools 제목에 `v0.12.1`이 표시된다.
...
## v0.12.1 단층 하단 내비게이션·모바일 앱 셸

- [ ] 하단에 `프롬프트`·`검사`·`기록`·`비교`·`상세`·`검색` 여섯 아이콘과 짧은 라벨이 한꺼번에 보이고 현재 기능 하나만 선택 상태로 구분된다.
- [ ] 각 항목을 한 번 누르면 각각 `전송 프롬프트`·`규칙 검사`·`기록`·`변경 비교`·`요청 상세`·`프롬프트 검색`으로 바로 이동하며 중간 그룹이나 조건부 하위 탭이 나타나지 않는다.
- [ ] `전송 프롬프트` 첫 화면에서 이번 요청의 총 토큰·컨텍스트 사용률·남은 컨텍스트·provider/model과 현재 요청 선택기를 한 장에서 확인할 수 있다.
- [ ] `기록`과 `비교`, `상세`와 `검색`을 번갈아 열어도 다른 기능의 마지막 선택을 기억해 엉뚱한 화면을 열지 않는다.
...
- [ ] 보관 수·불러올 수·개인정보·테마처럼 자주 쓰는 설정은 바로 보이고, 드물게 쓰는 고급 설정만 사용자가 펼친다.
- [ ] 백업·가져오기·저장소 정리 기능은 `데이터 도구` 한 진입점에서 찾을 수 있으며 설정 첫 화면에 여러 개의 동급 버튼으로 흩어져 있지 않다.
- [ ] 검사 탭에서 AI 의미 검사 opt-in을 찾을 수 있고, 이 스위치나 버튼을 켜는 것만으로 전송 미리보기·실행 상태·새 AI 결과가 생기지 않는다.
...
- [ ] 패널 폭을 430px 이하로 줄여도 하단의 여섯 기능이 한꺼번에 구분되며 가로 스크롤이나 잘린 글자가 없다.
...
v0.13.0은 익명화 평가 corpus, AI 제안·근거 품질 회귀, provider 호환성과 v0.12.1 실사용에서 발견된 UI/UX 불편을 보강하는 방향입니다. 현재 빠른 시작과 필드별 tooltip은 완료된 온보딩 튜토리얼이 아닙니다. 코치마크·워크스루는 이번 정보 구조와 접근성 검토 결과를 바탕으로 별도 버전에서 구체화합니다.
