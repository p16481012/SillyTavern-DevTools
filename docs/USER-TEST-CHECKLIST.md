# v0.12.2 사용자 실사용 체크리스트

v0.12.2는 독립 `요청 상세` 탭을 없애고 다섯 실제 기능만 고정 하단 내비게이션에 남기며, 보존할 생성 설정과 프롬프트 payload를 전송 프롬프트에 편입합니다. 아래에는 v0.12.2 프롬프트·검사 흐름과 모바일 앱 셸 검토, v0.11.4 이하 캡처·AI·저장 회귀 검토를 함께 적었습니다. AI 검사는 선택한 full snapshot의 프롬프트 원문을 선택한 프로필 또는 현재 provider에 보낼 수 있고 취소가 이미 시작된 계산·과금을 되돌린다고 보장하지 않으므로, 비밀이나 실제 개인정보가 없는 테스트 프롬프트와 작은 응답 상한을 사용하세요.

## 시작 전

- [ ] ST DevTools 제목에 `v0.12.2`가 표시된다.
...
v0.13.0은 익명화 평가 corpus, AI 제안·근거 품질 회귀, provider 호환성과 v0.12.2 실사용에서 발견된 UI/UX 불편을 보강하는 방향입니다. 현재 빠른 시작과 필드별 tooltip은 완료된 온보딩 튜토리얼이 아닙니다. 코치마크·워크스루는 이번 정보 구조와 접근성 검토 결과를 바탕으로 별도 버전에서 구체화합니다.
