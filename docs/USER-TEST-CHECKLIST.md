# v0.11.1 사용자 실사용 체크리스트

v0.11.0은 기본값이 꺼진 선택적 AI Semantic Inspector를 추가했고, v0.11.1은 장문 일반 요청의 캡처 회귀를 수정하면서 작업 중심 탐색과 빠른 시작 UI를 추가합니다. 아래 항목은 실제 SillyTavern의 현재 provider/model, `generateRaw`, 채팅 캡처, IndexedDB와 브라우저 UI에서 확인해야 하는 검토 절차입니다. AI 검사는 선택한 full snapshot의 프롬프트 원문을 현재 provider에 보낼 수 있고 취소가 이미 시작된 계산·과금을 되돌린다고 보장하지 않으므로, 비밀이나 실제 개인정보가 없는 테스트 프롬프트와 작은 응답 상한을 사용하세요. 메모리 cache도 전체 raw 요청/응답과 전용 source/quote 필드는 보관하지 않지만 AI가 원문 표현을 반복한 title·summary·rationale를 TTL 동안 유지할 수 있습니다.

## 시작 전

- [ ] ST DevTools 제목에 `v0.11.1`이 표시된다.
...
- [ ] AI 의미 검사를 실행하지 않은 일반 요청 한 건을 보내면 `캡처 중 → 처리 중 → 저장됨` 흐름 뒤 현재 채팅에 새 스냅샷이 보인다.
...
- [ ] 첫 스냅샷이 없는 채팅에서 빠른 시작 단계가 나타나고, 상단 도움말을 열고 닫은 뒤 원래 조작 위치로 돌아갈 수 있다.
- [ ] 규칙 검사 진입 직후 검사 결과가 설정·비교 정책·AI 의미 검사보다 먼저 보이고, 고급 영역은 사용자가 펼치기 전 대량 내용을 렌더링하지 않는다.
- [ ] 검색 필터, 타임라인 저장 관리와 설정의 고급 항목이 접혀 있으며 제목·요약만으로 용도를 구분하고 필요할 때 각각 펼칠 수 있다.
...
v0.12.0은 새 대형 편집 기능보다 익명화 평가 corpus, AI 제안·근거 품질 회귀, provider 호환성과 모바일·접근성 품질을 보강하는 방향입니다. 코치마크·워크스루는 이 체크리스트의 v0.11.1 실사용 피드백으로 작업 구분과 도움말 진입점이 적절한지 확인한 뒤 구체화합니다.
