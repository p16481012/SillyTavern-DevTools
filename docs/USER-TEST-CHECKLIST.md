# v0.12.0 사용자 실사용 체크리스트

v0.12.0은 기존 캡처·검사 안전 경계를 유지하면서 처음 사용하는 사람을 위한 작업 순서, 설정, 테마와 모바일 비교 화면을 정리합니다. 아래에는 v0.12.0 UI/UX 검토와 v0.11.4 이하 캡처·AI·저장 회귀 검토를 함께 적었습니다. AI 검사는 선택한 full snapshot의 프롬프트 원문을 선택한 프로필 또는 현재 provider에 보낼 수 있고 취소가 이미 시작된 계산·과금을 되돌린다고 보장하지 않으므로, 비밀이나 실제 개인정보가 없는 테스트 프롬프트와 작은 응답 상한을 사용하세요.

## 시작 전

- [ ] ST DevTools 제목에 `v0.12.0`이 표시된다.
...
## v0.12.0 초보자 중심 UI/UX

- [ ] 상단에 아이콘이 있는 `프롬프트`·`검사`·`기록`·`도구` 네 작업 탭이 보이고, 현재 탭만 선택 상태로 구분된다.
- [ ] 각 작업에 처음 들어갔을 때 가장 중요한 다음 행동 하나가 먼저 보이며, 고급 설정·상세 데이터가 초기 화면을 밀어내지 않는다.
...
v0.13.0은 익명화 평가 corpus, AI 제안·근거 품질 회귀, provider 호환성과 v0.12.0 실사용에서 발견된 UI/UX 불편을 보강하는 방향입니다. 현재 빠른 시작과 필드별 tooltip은 완료된 온보딩 튜토리얼이 아닙니다. 코치마크·워크스루는 이번 정보 구조와 접근성 검토 결과를 바탕으로 별도 버전에서 구체화합니다.
